...
static Fsm_GuardType MenuManager_AutoReheatWhenLow_Entry              (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);
static Fsm_GuardType MenuManager_AutoReheatWhenLow_Exit               (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);
static Fsm_GuardType MenuManager_AutoReheatWhenLow_StartBut           (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);
static Fsm_GuardType MenuManager_AutoReheatWhenLow_StopBut            (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);
static Fsm_GuardType MenuManager_AutoReheatWhenLow_UpBut              (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);
static Fsm_GuardType MenuManager_AutoReheatWhenLow_DownBut            (Fsm_ContextStructPtr const pFsmContext, Fsm_EventType event);

/** Menu manager state machine */
Fsm_EventEntryStruct MenuManager_AutoReheatWhenLow_StateMachine[7] =
{
  ...
  if (MenuManager_AutoReheatWhenLow_InternalState == MENUMANAGER_AUTOREHEATWHENLOW_INTERNALSTATE_EXITING)
  {
    ...
    if (MenuManager_AutoReheatWhenLow_Counter >= MENUMANAGER_AUTOREHEATWHENLOW_COUNTER_MAX)
    {
      ...
      Fsm_TriggerEvent( &MenuManager_FsmContext, \
                        (Fsm_EventType)((*(MenuManager_AutoReheatWhenLow_ChildMenuConf.childMenuCfg))[MenuManager_AutoReheatWhenLow_ListIndex].childMenuEvent));
    }
  }
}
