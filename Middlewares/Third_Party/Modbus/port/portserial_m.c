...
/* ----------------------- Macro definitions --------------------------------*/
#define MODBUS_MASTER_SERIAL                &huart1
...
BOOL xMBMasterPortSerialInit(UCHAR ucPORT, ULONG ulBaudRate, UCHAR ucDataBits, eMBParity eParity)
{
  ...
  {
    MODBUS_DEBUG("Master event_serial Event creat success id=%d\r\n", event_serial);
  }
  else
  {
    MODBUS_DEBUG("Master event_serial Event creat faild err=%d\r\n", event_serial);
  }
  
  /* Create master send task */
  BaseType_t xReturn = xTaskCreate((TaskFunction_t)serial_soft_trans_irq,          /* Task entry function */
                                   (const char *)"master trans",                   /* Task name */
 ...
 * processor. This function should then call pxMBMasterFrameCBByteReceived( ). The
...
/**
  ...
  * @retval None
  */
void Master_RxCpltCallback(UART_HandleTypeDef *huart)
{
  int ch = -1;
